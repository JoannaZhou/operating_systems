...
/* Global variable for the thread/pcb table. */
struct process * p_table[MAXTHREADS];
...
{
	//LOOP THROUGH T_TABLE TO FIND EMPTY SPOTS
	int i;

	for(i=1; i<MAXTHREADS ; i++){
		//an empty spot/pid is found
		if(p_table[i]==NULL){
			//kprintf("able to make new process\n");
			...
			thread->t_pid=i; //the pid is the index on the table
			
			//initialize thread so that it has no children yet

			process->me=thread;
			if(curthread!=NULL){
				process->parent_pid= curthread->t_pid;
			}
			else{
			...
			process->exitcode= -1;
			char name = (char) i ;
			process->exit_sem=sem_create(&name, 0);
			
			p_table[i]=process;
			//kprintf("new process made. Index is %d, pid is: %d\n", i, p_table[i]->me->t_pid);
			return thread;
		}
	}
	return NULL;
...
{
	//DEBUG(DB_THREADS, "[THREAD] Destroying a thread\n");
...
//	kfree(p_table[thread->t_pid]->exit_sem);
//	kfree(p_table[thread->t_pid]);
//	p_table[thread->t_pid]=NULL;
...
{
	int i, result;
	//DEBUG(DB_THREADS, "[THREAD] Removing the zombies\n");
	assert(curspl>0);
...
thread_killall(void)
{
	...
	//DEBUG(DB_THREADS, "[THREAD] Killing all sleeping threads\n");
	assert(curspl>0);
...
thread_panic(void)
{
	//DEBUG(DB_THREADS, "[THREAD] A panic ocurrs, shutting down all other threads\n");
	assert(curspl > 0);
...
{	
	...
	//DEBUG(DB_THREADS, "[THREAD] Initializing new thread\n");

	/* Create the data structures we need. */
	sleepers = array_create();
	if (sleepers==NULL) {
		//DEBUG(DB_THREADS, "[THREAD] sleepers is NULL\n");
		panic("Cannot create sleepers array\n");
	}

	zombies = array_create();
	if (zombies==NULL) {
		//DEBUG(DB_THREADS, "[THREAD] zombies is NULL\n");
...
thread_shutdown(void)
{
	...
	//DEBUG(DB_THREADS, "[THREAD] Thread final cleanup\n");
	    ...
	    struct thread **ret)
{
	...
	//DEBUG(DB_THREADS, "[THREAD] Creating new thread with name %s", name); 
