 ...
 * manage.e that spans multiple frames? Then we would reserve however many frames for that structure. But what happens when we delete it? A well-formed kfree should point to the beginning frame of that structure, so should we store the amount of contiguous frames each kmalloc asked for?

What about thread identifiers? Should thread A be able to kfree something in
 */
void
ram_getsize(u_int32_t *lo, u_int32_t *hi)
{
	*lo = firstpaddr;
	*hi = lastpaddr;
	firstvaddr = PADDR_TO_KVADDR(firstpaddr);
