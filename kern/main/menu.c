...
common_prog(int nargs, char **args)
{
	...
	//my_args = copy_arguments(nargs, args);
	result = thread_fork(args[0] /* thread name */,
			args /* thread arg */, nargs /* thread arg */,
			cmd_progthread, NULL);
	if (result) {
		kprintf("thread_fork failed: %s\n", strerror(result));
		//free_arguments(nargs, my_args);
		return result;
	}
...
/*
 * Command for turning on and off debug flags
 */
static
void
show_dbflags_menu(const char *name, const char *x[])
{
	int ct, half, i;
...
static const char *dbflags_menu[] = {
	"[df 1 on/off]        DB_LOCORE        ",
	"[df 2 on/off]        DB_SYSCAL        ",
...
static int cmd_dbflags(int nargs, char **args)
{
	show_dbflags_menu("OS/161 Debug flags", dbflags_menu);
	kprintf("Current value of dbflags is %#x\n", dbflags);

	return 0;
}

static void bitmanip(int _flagnum, int on, int off){
	
	
	u_int32_t bit= dbflags >> (_flagnum-1);
	bit&= 0x01;
	
	u_int32_t flagconst= 0x01 << (_flagnum-1);
	
	if(on==0){
		if(bit==0){
			dbflags= dbflags + flagconst;
			//DEBUG(DB_THREADS, "turned %d on. dbflags is now: 0x%03x\n", _flagnum, dbflags);
		}
		else if(bit==1){
			//DEBUG(DB_THREADS, "df %d is already turned on\n", _flagnum);
		}
	}
	else if (off==0){
		if(bit==1){
			dbflags= dbflags - flagconst;
			//DEBUG(DB_THREADS, "turned %d off. dbflags is now: 0x%03x\n", _flagnum, dbflags);
		}
		else if(bit==0){
			//DEBUG(DB_THREADS, "df %d is already turned off\n", _flagnum);
		}
	}
}

static int cmd_setdbflags(int nargs, char **args){	
	...
	//Retrieve flag number and mode
	int flagnum = atoi(args[1]);
	int mode_on= strcmp(args[2], "on");
	int mode_off= strcmp(args[2], "off");
	
	//DEBUG(DB_THREADS, "flagnum: %d, mode_on: %d, mode_off: %d\n", flagnum, mode_on, mode_off);
	
	//check for invalid input
	if (flagnum > 12 || flagnum < 1){
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
	}
	if(mode_on !=0 && mode_off !=0){
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
	}

	//set the flag
	bitmanip(flagnum,mode_on,mode_off);
